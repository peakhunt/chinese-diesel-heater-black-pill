...
heater_init(void)
{
  ...
  glow_plug_init(&_heater.glow_plug, gpio_out_pin_pc14);
  oil_pump_init(&_heater.oil_pump, gpio_out_pin_pc15);
  fan_init(&_heater.fan, pwm_channel_0, gpio_in_pin_pb0);

  //
  // 3.3V source. 51K R2 in voltage divider circuit
  //
  ntc50_init(&_heater.outlet_temp, 3.3, 51 * 1000.0f, adc_channel_0);
  ntc50_init(&_heater.room_temp, 3.3, 51 * 1000.0f, adc_channel_1);
